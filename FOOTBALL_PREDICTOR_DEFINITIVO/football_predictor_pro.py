...
def main():
    ...
    if menu == "🏠 Dashboard":
        ...
        with tab_summary:
            ...
            with c2:
                total_goals = dm.session.query(pd.func.sum(PlayerSeasonStats.goals)).filter(PlayerSeasonStats.season == active_season).scalar() or 0
        ...
        with tab_history:
            ...
            for s in seasons_all:
                g = dm.session.query(pd.func.sum(PlayerSeasonStats.goals)).filter(PlayerSeasonStats.season == s).scalar() or 0
        ...
        selected_teams = st.multiselect("Selecciona 8 equipos para la simulación", 
        ...
        if st.button("🚀 Iniciar Gran Simulación"):
            ...
            result = simulator.run_full_tournament(sim_teams)
            
            st.balloons()
            st.success(f"🏆 ¡EL CAMPEÓN PREDICHO ES: {result['champion']['name']}!")
            
            # Mostrar cuadro del torneo
            for r_name, r_teams in result['rounds_history'].items():
                with st.expander(f"Ver {r_name}"):
                    # Corregir lógica de visualización de emparejamientos
                    matches = []
                    for i in range(0, len(r_teams), 2):
                        if i + 1 < len(r_teams):
                            matches.append(f"🏟️ **{r_teams[i]}** vs **{r_teams[i+1]}**")
                        else:
                            matches.append(f"🏟️ **{r_teams[i]}** (Pasa directo)")
                    st.write("  \n".join(matches))
