...
class DixonColesModel(BaseFootballModel):
    ...
    def predict_match(self, home: Dict, away: Dict, **kwargs) -> Dict:
        ...
        Wrapper de match_probabilities.
        """
        home_form = kwargs.get('home_form', 1.0)
        away_form = kwargs.get('away_form', 1.0)
        result = self.match_probabilities(home, away, home_form, away_form)
        # Asegurar que top_scores tenga el campo "score" como cadena para compatibilidad
        for score in result['top_scores']:
            if 'score' not in score:
                score['score'] = f"{score['home_goals']}-{score['away_goals']}"
        return result
