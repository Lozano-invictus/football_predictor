...
try:
    ...
    # Fallback simple si thefuzz no está disponible
    process = None
    fuzz = None
    
    def simple_fuzzy_match(query, choices):
        """Simple fallback matching if thefuzz isn't available."""
        query_lower = query.lower()
        for choice in choices:
            if query_lower in choice.lower() or choice.lower() in query_lower:
                return (choice, 80)  # Confianza baja pero funcional
        return (choices[0] if choices else None, 0) if choices else (None, 0)
...
class DataNormalizer:
    ...
    def get_team_id(self, name):
        ...
        if process and fuzz:
            ...
            match, score = simple_fuzzy_match(name, names)
            if score > 80:
                return self.team_cache[match]
        return None

    def get_player_id(self, name):
        """Resuelve el ID de un jugador por nombre, con fuzzy matching."""
        if not name or pd.isna(name): return None
        name = name.strip()

        # 1. Caché directo
        if name in self.player_cache: return self.player_cache[name]

        # 2. Fuzzy match o fallback
        names = list(self.player_cache.keys())
        if not names: return None
        if process and fuzz:
            match, score = process.extractOne(name, names, scorer=fuzz.token_sort_ratio)
            if score > 85:
                return self.player_cache[match]
        else:
            match, score = simple_fuzzy_match(name, names)
            if score > 80:
                return self.player_cache[match]
...
class ETLMasterPro:
    ...
    def normalize_season(self, s_folder: str) -> str:
        """Convierte '2022_23' a '2022-23'"""
        return s_folder.replace("_", "-")
    ...
    def _import_master_data(self, session: Session):
        # Importar datos maestros desde la carpeta _MASTER
        if not self.master_folder.exists():
            self.log_audit("Carpeta _MASTER no encontrada. Saltando datos maestros...", "WARNING")
            return
        
        # Importar todos los equipos de todas las temporadas si existe
        all_teams_path = self.master_folder / "all_teams_all_seasons.csv"
        if all_teams_path.exists():
            self.log_audit("Cargando equipos maestros desde _MASTER...")
            df_all_teams = pd.read_csv(all_teams_path)
            for _, row in df_all_teams.iterrows():
                name = row.get('Team') or row.get('team')
                if not name: continue
                team = session.query(Team).filter_by(name=name).first()
                if not team:
                    team = Team(
                        ...
                        league=row.get('League', 'Champions League'),
                        ucl_titles=int(row.get('UCL Titles', 0)),
        ...
        # Importar historial de ganadores
        winners_path = self.master_folder / "winners_history.csv"
        if winners_path.exists():
            self.log_audit("Cargando historial de ganadores UCL desde _MASTER...")
            # Aquí podríamos importar ganadores históricos si fuera necesario
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ETL Maestro UCL")
    parser.add_argument(
        "--base",
        default=os.getenv(
            "CHAMPIONS_DATA_PATH",
            str(Path(__file__).parent.parent.parent / "CHAMPIONS_LEUE_PRO")
        ),
        help="Ruta a la carpeta CHAMPIONS_LEUE_PRO"
    )
    args = parser.parse_args()
    print(f"📂 Usando base de datos en: {args.base}")
    etl = ETLMasterPro(args.base)
